package ua.lpnu.train.wagon;

import junit.framework.TestCase;

import static org.junit.Assert.assertNotEquals;

public class CargoTest extends TestCase {

    Wagon[] wagons = new Wagon[3];

    public CargoTest() {
        wagons[0] = new Cargo("01", 40, 50, 30);
        wagons[1] = new Cargo("02", 120, 60, 40);
        wagons[2] = new Cargo("03", 80, 40, 25);
    }

    public void testGet() {
        assertEquals(wagons[0].getString(), wagons[0].get().getString());
        assertNotEquals(wagons[2].getName(), wagons[1].get().getName());
    }

    public void testGetString() {
        assertEquals("Вагон: 01 Вага: 50 т. Швидкість: 40 км./год. Місткість: 30 т.", wagons[0].getString());
        assertEquals("Вагон: 02 Вага: 60 т. Швидкість: 120 км./год. Місткість: 40 т.", wagons[1].getString());
        assertEquals("Вагон: 03 Вага: 40 т. Швидкість: 80 км./год. Місткість: 25 т.", wagons[2].getString());
    }

    public void testGetName() {
        assertEquals("01", wagons[0].getName());
        assertEquals("02", wagons[1].getName());
        assertEquals("03", wagons[2].getName());
    }

    public void testGetWeight() {
        assertEquals(50, wagons[0].getWeight());
        assertEquals(60, wagons[1].getWeight());
        assertEquals(40, wagons[2].getWeight());
    }

    public void testGetSpeed() {
        assertEquals(40, wagons[0].getSpeed());
        assertEquals(120, wagons[1].getSpeed());
        assertEquals(80, wagons[2].getSpeed());
    }

    public void testGetCapacity() {
        assertEquals(30, wagons[0].getCapacity());
        assertEquals(40, wagons[1].getCapacity());
        assertEquals(25, wagons[2].getCapacity());
    }
}